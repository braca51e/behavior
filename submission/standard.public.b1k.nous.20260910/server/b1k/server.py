...
async def _ws_handler(server: B1KServer, conn) -> None:
    """websockets.asyncio server handler: reset, then obs<->action loop.

    The evaluator sends *binary* msgpack obs frames.  We also accept an
    optional JSON control frame (text, or JSON-in-bytes) for ``reset`` /
    ``set_task`` — used by local harnesses, never by the real evaluator (which
    resets via a WS (re)connect).  A binary frame is treated as a control JSON
    only if it cleanly decodes to a JSON object with a ``type`` key; otherwise
    it is an obs payload.
    """
