...
# ---------------------------------------------------------------------------
# msgpack pack/unpack — compatible with omnigibson.eval.utils.network_utils
# (numpy ndarray extension used by WebsocketPolicy / serve_b1k.py)
# ---------------------------------------------------------------------------
...
def pack(obj: Any) -> bytes:
    """msgpack-encode ``obj`` (dicts / lists / numpy arrays)."""
    return msgpack.packb(obj, use_bin_type=True, default=_pack_data)


def unpack(data: bytes | bytearray | memoryview) -> Any:
    """msgpack-decode ``data`` into plain Python (dicts, lists, numpy arrays)."""
    if isinstance(data, memoryview):
        data = data.tobytes()
    return msgpack.unpackb(bytes(data), raw=False, object_hook=_unpack_data)
...
def frame_from_dict(obs: dict) -> Frame:
    ...
    for key, value in obs.items():
        ...
        is_image3 = arr.ndim == 3 and arr.shape[2] in (1, 3, 4)
        if role and mod is None and is_image3:
            mod = "rgb" if arr.shape[2] in (3, 4) else "depth"
        ...
        if mod == "rgb" and is_image3 and role:
            ...
            if getattr(frame, target, None) is None:
                ...
                if a.shape[2] == 1:
                    ...
                    # OmniGibson RGB cameras often ship RGBA; drop alpha.
                    a = a[..., :3]
...
def encode_action(action: np.ndarray) -> bytes:
    ...
    The official ``WebsocketClientPolicy`` does
    ``torch.from_numpy(action_dict["action"])``, so ``action`` must be a
    numpy ndarray (packed via the ndarray extension), not a Python list.
    """
